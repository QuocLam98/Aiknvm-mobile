...
class AdminTableScaffold extends StatefulWidget {
  final List<String> columns; // without index column, actions added automatically at end if actionsBuilder!=null
  final List<List<String>> data; // each row: list of cell strings for the columns
...
class _AdminTableScaffoldState extends State<AdminTableScaffold> {
              ...
              child: SingleChildScrollView(
                child: DataTable(
                  ...
                  columns: [
                    ...
                    if (hasActions) const DataColumn(label: SizedBox(width: 80)),
                  ],
                  rows: [
                    for (int i = 0; i < pageIndexes.length; i++)
                      _buildRow(index: startIndex + i + 1, rowIndex: pageIndexes[i], hasActions: hasActions),
                  ],
                ),
              ),
            ),
  ...
  DataRow _buildRow({required int index, required int rowIndex, required bool hasActions}) {
  ...
  Widget _pagination(int totalPages) {
    List<Widget> buttons = [];
    buttons.add(_pagerButton('Prev', enabled: _page > 1, onTap: () => setState(() => _page--)));
    ...
    if (totalPages <= maxPagesToShow) {
      ...
      final pages = <int>{1, totalPages, _page, _page - 1, _page + 1, 2, totalPages - 1}
    ...
    buttons.add(_pagerButton('Next', enabled: _page < totalPages, onTap: () => setState(() => _page++)));
  ...
  Widget _pagerButton(String label, {required bool enabled, required VoidCallback onTap}) =>
      ElevatedButton(onPressed: enabled ? onTap : null, child: Text(label));

  Widget _pagerNumber(int p) =>
      OutlinedButton(onPressed: () => setState(() => _page = p), child: Text(p.toString()));
